<?PHP
/*------------------------------------------------------------------------
# remooz.php for PLG - SYSTEM - IMAGESIZER
# ------------------------------------------------------------------------
...
$path="plugins"."/"."system"."/"."imagesizer"."/"."lbscripts"."/"."remooz"."/";

$lang = JFactory::getLanguage();
$l=substr($lang->getTag(),0,2);
$document   = JFactory::getDocument();

if(file_exists(JPATH_SITE."/".$path.$l.'_ReMooz.css')){
	$document->addStyleSheet($path.$l.'_ReMooz.css','text/css',"screen");	
}else{
	$document->addStyleSheet($path.'ReMooz.css','text/css',"screen");
}

if(file_exists(JPATH_SITE.DS.$path.$l.'ReMooz.js')){
	$document->addScript($path.$l.'ReMooz.js');
}else{
	$document->addScript($path.'ReMooz.js');	
}
unset($path);
